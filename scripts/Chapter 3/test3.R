...
hist_plot <- ggplot(diamonds, aes(x = price)) +
  geom_histogram(aes(y= after_stat(density)), binwidth = 500, fill = "steelblue", color = "white") + geom_density(color = "red", size = 1.2, fill = "orange", alpha = 0.2) +
...
box_plot <- ggplot(diamonds, aes(x = price, y = 1)) +
  ...
  theme_krul() + stat_boxplot(geom = "errorbar", width = 0.3, size = 0.5)
