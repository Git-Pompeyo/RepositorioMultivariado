...
\item \textbf{Kolmogorov–Smirnov.} Compara la función de distribución empírica con la acumulada de la normal. Es no paramétrica y aplicable a cualquier tamaño, pero menos potente que Shapiro–Wilk en muestras pequeñas. Para llamar a esta función en R, se puede usar el comando \texttt{ks.test()}.
...
\item \textbf{Jarque–Bera.} Basada en la asimetría y curtosis de los datos. Es potente en muestras grandes, pero en muestras pequeñas puede ser demasiado sensible a leves desvíos. En R, se puede llamar utilizando la función \texttt{jarque.bera.test()} del paquete \texttt{tseries}.
\end{enumerate}

Como ilustración, aplicaremos estas pruebas a las muestras con sesgo a la izquierda, simétrica y con sesgo a la derecha que generamos anteriormente.
...
Notemos que todas estas pruebas rechazan la hipótesis de normalidad de manera contundente para las muestras con sesgo a la izquierda y con sesgo a la derecha, mientras que no rechazan esta hipótesis para la muestra simétrica. En general, todas estas pruebas suelen rechazar la hipótesis nula para muestras muy grandes, incluso si la desviación de la normalidad es mínima. Es por esto que es importante complementar los resultados de estas pruebas con gráficos QQ y análisis adicionales para tomar decisiones informadas sobre la viabilidad de nuestros datos. Como ejemplo, consideraremos ahora estas mismas pruebas aplicadas a muestras mucho más pequeñas extraídas de las mismas distribuciones consideradas anteriormente.
```{r}
...
Notemos que en este caso, las pruebas de normalidad no logran rechazar la hipótesis de normalidad para ninguna de las muestras consideradas. Esto ilustra cómo el tamaño de la muestra puede influir significativamente en los resultados de estas pruebas, y refuerza la importancia de considerar el contexto del análisis estadístico al interpretar estos resultados. Como ejemplo adicional, consideraremos ahora estas mismas pruebas aplicadas a las muestras leptocúrtica, mesocúrtica y platicúrtica que generamos anteriormente.
