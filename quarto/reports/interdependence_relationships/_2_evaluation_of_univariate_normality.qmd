...
Como ilustración, aplicamos estas pruebas a las muestras con sesgo a la izquierda, simétrica y con sesgo a la derecha que generamos anteriormente.
...
Notemos que todas estas pruebas rechazan la hipótesis de normalidad de manera contundente para las muestras con sesgo a la izquierda y con sesgo a la derecha, mientras que no logran rechazar esta hipótesis para la muestra simétrica. En general, todas estas pruebas suelen rechazar la hipótesis de normalidad para muestras muy grandes, incluso si la desviación de la normalidad es mínima. Es por esto que es importante complementar los resultados de estas pruebas con gráficos QQ y análisis adicionales para tomar decisiones informadas sobre la viabilidad de nuestros datos. Como ejemplo, consideramos ahora estas mismas pruebas aplicadas a muestras mucho más pequeñas extraídas de las mismas distribuciones consideradas anteriormente.
```{r}
#| tbl-cap: "Tabla de valores $p$ de las pruebas de normalidad aplicadas a las muestras pequeñas generadas a partir de distribuciones con sesgo a la izquierda, simétrica y con sesgo a la derecha." 
#| label: tbl-small-skewness-normality
#| tbl-pos: H
#| message: false
...
left_skew_sample_tbl <- tibble(sample = left_skew_sample)
symmetric_sample_tbl <- tibble(sample = symmetric_sample)
right_skew_sample_tbl <- tibble(sample = right_skew_sample)
...
Notemos que en este caso, las pruebas de normalidad no logran rechazar la hipótesis nula de normalidad para ninguna de las muestras consideradas. Esto ilustra cómo el tamaño de la muestra puede influir significativamente en los resultados de estas pruebas, y refuerza la importancia de considerar el contexto del análisis estadístico al interpretar estos resultados. Como ejemplo adicional, consideremos ahora estas mismas pruebas aplicadas a las muestras leptocúrtica, mesocúrtica y platicúrtica que generamos anteriormente.
