...
La siguiente tabla resume las características poblacionales más relevantes de la distribución Gamma:
...
kable(gamma_tbl, booktabs = TRUE, escape = FALSE)
```
...
gamma_distribution_plot <- gamma_distribution_family_tbl |>
  ...
  scale_color_manual(
    ...
    labels = c(
      TeX("$\\alpha$ = 1.5"), TeX("$\\alpha$ = 2"),
      TeX("$\\alpha$ = 3"), TeX("$\\alpha$ = 4"),
      TeX("$\\alpha$ = 6"), TeX("$\\alpha$ = 8")
    )
  ) +
  labs(
...
Las características poblacionales más relevantes de la distribución normal generalizada se muestran en la siguiente tabla:
...
generalized_normal_tbl <- tibble(
  ...
  `Valor` = c(
    ...
    "0",
    "$\\frac{\\Gamma(5/\\beta) \\Gamma(1/\\beta)}{\\Gamma(3/2)^{2}} - 3$"
  )
)
