...
# Generamos las gráfica de la muestra con sesgo a la izquierda
...
# Generamos las gráfica de la muestra simétrica
...
# Generamos las gráfica de la muestra con sesgo a la derecha
...
$$ 
El paquete `e1071`, a través de la función `kurtosis()`, nos permite calcular dos tipos de curtosis a partir de esta definición: la \emph{curtosis muestral de tipo 1} 
$$
\widetilde{G}_{2} = \frac{\widetilde{M}_4}{\widetilde{S}^4} - 3 = \frac{\frac{1}{n} \sum_{i=1}^n (X_i - \bar{X})^4}{\left( \frac{1}{n} \sum_{i=1}^n (X_i - \bar{X})^2 \right)^{2}} - 3,
$$ 
y la \emph{curtosis muestral de tipo 2} 
$$
G_{2} = \frac{n(n+1)}{(n-1)(n-2)(n-3)} \sum_{i=1}^n \left( \frac{X_i - \bar{X}}{S} \right)^4 - \frac{3(n-1)^2}{(n-2)(n-3)}. 
$$ 
Notemos que ambas definiciones estiman el exceso de curtosis. Al igual que con la asimetría muestral, no existe un estimador insesgado de la curtosis poblacional, pero ambas versiones de la curtosis muestral son consistentes.
