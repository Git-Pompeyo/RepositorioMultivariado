...
\begin{itemize} 
\item Los vectores de medias $\mu_k$ para cada clase $k=1,\ldots,K$. 
\item La matriz de covarianza común $\Sigma$. 
\item Los pesos a priori $\pi_k$ para cada clase $k=1,\ldots,K$.
\end{itemize} 
...
\begin{itemize}
\item Las medias de cada clase se estiman como el promedio de los vectores de características dentro de cada clase:
$$
\hat{\mu}_k = \frac{1}{n_k} \sum_{i| g_i = k} x_i, 
$$ 
donde $n_k$ es el número de observaciones en la clase $k$, y estmos sumando sobre todos los índices $i$ tales que la etiqueta de clase $g_i$ es igual a $k$.
\item La matriz de covarianza común se estima como la media ponderada de las matrices de covarianza dentro de cada clase: 
$$
\hat{\Sigma}_{\text{MLE}} = \frac{1}{n - K} \sum_{k=1}^K \sum_{i| g_i = k} (x_i - \hat{\mu}_k)(x_i - \hat{\mu}_k)^T,
$$ 
\item Los pesos a priori se estiman como la proporción de observaciones en cada clase: 
$$ 
\hat{\pi}_k = \frac{n_k}{n} 
$$
\end{itemize}

Este enfoque es el que se utiliza comúnmente en la literatura estadística y que se implementa en R a través de la función \texttt{lda()} del paquete \texttt{MASS}.
...
\begin{itemize}
\item Las medias de cada clase se estiman de la misma manera que en el método FE:
$$
\hat{\mu}_k = \frac{1}{n_k} \sum_{i| g_i = k} x_i
$$
\item La matriz de covarianza común se estima como:
$$
\hat{\Sigma}_{\text{MLE}} = \frac{1}{n} \sum_{k=1}^K \sum_{i| g_i = k} (x_i - \hat{\mu}_k)(x_i - \hat{\mu}_k)^T
$$
\item Los pesos a priori se estiman como: 
$$
\hat{\pi}_k = \frac{n_k}{n}
$$
\end{itemize}

Notemos que en este caso, el estimador de la matriz de covarianza común es sesgado, pero consistente. Este enfoque es más común en la literatura de aprendizaje automático y se utiliza en implementaciones como la función \texttt{LinearDiscriminantAnalysis} del paquete \texttt{scikit-learn} en Python.

### Comparación de los enfoques FE y MLE 

Notemos que la única diferencia entre estos dos enfoques está dada en la forma en la que estimamos la matriz de covarianza común $\Sigma$. De manera más precisa, tenemos que:
$$
...
\bottomrule
\end{tabular}
\caption{Comparación de los enfoques FE y MLE para el ajuste del modelo discriminante gaussiano.} 
\end{table}
