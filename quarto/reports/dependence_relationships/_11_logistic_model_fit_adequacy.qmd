...
$$
para algunos parámetros $b_{0}, b_{1}, \ldots, b_{p}$. En este capítulo estudiaremos el problema de ajustar este modelo a un conjunto de datos y verificar su adecuación. Notemos que una vez que disponemos de un modelo ajustado y verificado, podemos utilizarlo para predecir la probabilidad de que $Y_{0} = 1$ dado un valor específico de $X_{0} = x_{0}$. A partir de esta información, podemos utilizar el clasificador de Bayes para asignar una clase a la observación $X_{0} = x_{0}$, lo cual es una de las aplicaciones principales de la regresión logística.

## Ajuste del modelo de regresión logística

Para el ajuste del modelo de regresión logística, utilizamos el método de máxima verosimilitud. Supongamos que disponemos de un conjunto de datos $\{(x_{i}, y_{i})\}_{i=1}^{n}$, donde $x_{i} = (x_{i\, 1}, x_{i\, 2}, \ldots, x_{i\, p})$ es el vector de variables explicativas para la $i$-ésima observación y $y_{i} \in \{0, 1\}$ es la variable de respuesta correspondiente. La función de verosimilitud para este conjunto de datos es: 
$$ 
