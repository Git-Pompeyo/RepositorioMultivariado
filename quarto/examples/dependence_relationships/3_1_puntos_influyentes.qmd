...
Donde: - $\hat{y}_j$ es el valor ajustado de la $j$-ésima observación usando todos los datos. - $\hat{y}_{j(i)}$ es el valor ajustado de la $j$-ésima observación cuando el $i$-ésimo punto de datos es omitido. - $p$ es el número de parámetros del modelo, incluyendo el intercepto. - $\text{MSE}$ es el error cuadrático medio del modelo ajustado:
...
Donde: - $r_i$ es el residuo estandarizado para el $i$-ésimo punto de datos. - $h_i$ es el valor de apalancamiento para el $i$-ésimo punto de datos.
...
$$
\mathbf{H} = \mathbf{\Phi}(\mathbf{\Phi}^\top \mathbf{\Phi})^{-1} \mathbf{\Phi}^\top
$$

Donde $\mathbf{\Phi}$ es la matriz de diseño del modelo de regresión. El apalancamiento para la $i$-ésima observación es el valor $h_{ii}$ en la diagonal de la matriz sombrero, y se calcula como:

$$
h_i = \mathbf{\Phi}_i (\mathbf{\Phi}^\top \mathbf{\Phi})^{-1} \mathbf{\Phi}_i^\top
$$
...
donde $p$ es el número de variables independientes del modelo, y $n$ es el número total de observaciones.
...
Donde: - $\beta_j$ es el coeficiente estimado usando todos los datos. - $\beta_{j(i)}$ es el coeficiente estimado después de eliminar la $i$-ésima observación. - $\text{SE}(\beta_j(i))$ es el error estándar del coeficiente estimado después de eliminar la $i$-ésima observación.
